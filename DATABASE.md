...
- **Soft deletion** (`deletedAt`) is used where history must survive: users, stores, products, categories, brands, collections, coupons, pages, reviews. A deleted store stays behind its orders and money records with a freed `deleted-<id>` slug, no owner and `formerOwnerId` set — orders cannot lose their store (`Restrict`), and the ledger, deposits and payouts would otherwise cascade away with it. Products that appear in orders are archived rather than deleted.
...
| Stores | `Store` (unique `slug` = subdomain; one per owner, `ownerId` null for the platform's demo store and for deleted stores, which keep `formerOwnerId`; status, pricing rule, branding, reserved `customDomain` and `stripeAccountId`), `StoreProduct` (the store's shelf: active flag, optional markup or fixed price; unique per store and product) |
| Store wallet | `WalletEntry` (append-only ledger; the balance is its sum. Each order posts `ORDER_SALE`, `ORDER_COMMISSION` and `ORDER_FULFILMENT` once — enforced by a unique `idempotencyKey` and a per-store advisory lock; refunds and cancellations add `ORDER_REFUND`, `COMMISSION_REVERSAL` and `FULFILMENT_REVERSAL` lines. `status` is `PENDING` (held) until the order is delivered, then `CLEARED`; only cleared money can be withdrawn. `ORDER_EARNING`/`ORDER_REVERSAL` are single net lines from before sales and costs were recorded separately), `Payout` (withdrawal: requested → approved → processing → paid, or rejected; the amount is held out of the available balance from the request), `Deposit` (transfer an owner declares — bank or crypto, with network, reference and an optional proof image; credited only when staff confirm it) |
...
| Promotions & reviews | `Coupon` (`storeId` null = platform coupon, usable only in the platform store), `CouponProduct`, `CouponCategory`, `CouponCustomer`, `CouponRedemption`, `Review`, `ReviewImage`, `StoreReview` (a customer's rating of a store: unique `orderId`, so one per delivered order; `isDemo` marks seeded sample reviews, which have no order; a database check keeps `rating` between 1 and 5) |
