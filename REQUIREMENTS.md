...
| Ledger | Append-only. Each order posts a **sale** (credit), **commission** (debit) and **fulfilment cost** (debit), each exactly once (idempotency key + a per-store database lock). Refunds and cancellations post compensating entries in proportion; nothing is edited. An order's entries are **pending** (held) until the order is **delivered**, whatever the payment method, and then **cleared**; at acceptance the part of the wholesale cost the customer's collected payment cannot cover (all of it for cash on delivery) is taken from the owner's available balance as a cleared entry. Only cleared money can be withdrawn, in USDT (TRC20). |
| Funding gate | **The store owner accepts every order themselves; nothing accepts one automatically** (not a payment, webhook, deposit or job). On Accept the wholesale cost is set aside once — **from the customer's payment when it has been collected**, so a normally priced paid order funds itself, and otherwise (cash on delivery, or a price below cost) from the owner's available balance. If that balance cannot cover it, Accept is refused ("Insufficient wallet balance to accept this order. Please add funds."), nothing is written, and the order keeps waiting; a confirmed deposit makes Accept possible but accepts nothing itself. |
