...
- **Rate limiting** — fixed windows in PostgreSQL (single atomic upsert, UTC timestamps), or in memory for tests; fails open on infrastructure errors. Limits per client address go through `rateLimitByIp`: the address comes from Vercel's own headers on Vercel, from `X-Forwarded-For` only when `TRUST_PROXY` is set, and when it is unknown the limit is skipped rather than shared by every visitor. Opening a store has limits of its own (`features/stores/opening-guard.ts`), apart from customer sign-ups: 20 openings an hour per network, and 10 wrong invitation codes in 15 minutes — only wrong codes count towards that one.
- **Deleting a store** (`features/stores/deletion.ts`) is a soft delete by design: orders refuse to lose their store (`onDelete: Restrict`) and the wallet ledger, deposits and payouts would cascade away with it. The store row stays as a record (`deletedAt`, a freed `deleted-<id>` slug, `formerOwnerId`) behind the retained orders and money records, while its shelf, bags, coupons, customer conversations and owner link are removed in one locked transaction. It is refused while the store has open orders, deposits under review, withdrawals in progress or a non-zero balance, and requires the store's address typed as confirmation, checked on the server.
