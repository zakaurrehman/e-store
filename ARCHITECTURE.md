...
7. **Ledger and funding** (`features/wallet/service.ts`). When the customer's money is in, the order's sale and commission are posted; `acceptOrderForFulfilment` then charges the fulfilment cost. Each posting carries an idempotency key and is written under a per-store PostgreSQL advisory lock (`pg_advisory_xact_lock`), and acceptance also locks the order row, so retries, duplicate webhooks and simultaneous clicks cannot post twice. If the balance cannot cover the charge the order becomes `AWAITING_FUNDS`; confirming a deposit re-runs acceptance for the store's waiting orders. Entries are `PENDING` until the money is collected (cash on delivery: on delivery) and `CLEARED` after; withdrawals only draw on cleared money. Refunds and cancellations post proportional compensating entries.
