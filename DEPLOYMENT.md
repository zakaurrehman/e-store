...
| `TRUST_PROXY` | `true` behind a reverse proxy you run, so client IPs come from `X-Forwarded-For`. Not needed on Vercel: there the client IP is read from the headers Vercel's edge sets, which clients cannot spoof. Without either, per-address rate limits are switched off (and a warning logged) rather than shared by every visitor. |
