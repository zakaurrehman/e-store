...
- **Support inbox** holds every conversation: messages to Zendropship (including owners' own questions), messages in owners' stores, and anything written from the **Customer Service** button that floats in the corner of the platform site, every storefront and the owner dashboard — all of it the same kind of conversation, answered the same way. Filter by where it came from (*To Zendropship*, *In owners' stores*, *Assigned to me*) and by status, and search by subject, customer, order number or store. A dot marks unread conversations; opening one marks it read.
