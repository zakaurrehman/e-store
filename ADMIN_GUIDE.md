...
**Add money** (in the store's *Money* panel) puts money into the owner's balance yourself — a transfer that arrived without the owner recording it, or money they are owed. Enter the amount that actually arrived, a reference if there is one, and what it is for; the dialog shows the balance before and after. It runs the same two steps as any deposit, recorded and then approved, so the owner sees it in their deposit history, the ledger gains exactly one entry, both steps are in the audit log under your name, and any of their orders waiting for funds go to fulfilment. Nothing here writes a balance directly.
...
- **Payment first.** An online order confirms itself when the payment provider's verified confirmation arrives; cash on delivery confirms at once. An order still **Awaiting payment** — a card that never went through, say, when the customer then paid by bank transfer — has a **Confirm payment** button in the queue and on its page: it records the payment, confirms the order, posts the sale to the owner's ledger and hands it to fulfilment in one step. Pressing it twice changes nothing. On an order that is already confirmed the same action is called **Mark as paid**.
...
- **Accepting for fulfilment** sets the wholesale cost aside, exactly once however often it is tried. It happens automatically as soon as an order is confirmed. When the customer has already paid, the cost comes out of that payment, so a normally priced paid order needs no deposit. A cash-on-delivery order has collected nothing yet, so its cost comes out of the owner's **available** balance at once. The order's timeline says which it was (*$300.00 wholesale cost set aside from the store balance*, say). Only if the available balance genuinely cannot cover its part does the order become **Awaiting funds**; the owner is emailed the shortfall, and it is accepted automatically when you confirm a deposit that covers it. **Accept for fulfilment** on the order page retries by hand. The owner's earnings on the order stay held until you mark it **Delivered**, which releases them in the same step. Orders in Zendropship's own store are never charged.
