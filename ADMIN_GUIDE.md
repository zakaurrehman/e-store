...
- **Delete** (in the list and on the store's page) removes a store permanently. The dialog lists what goes and what stays, and **Delete store permanently** only unlocks once you type the store's address exactly — the server checks it too. What goes: the store and its address (the storefront stops at once, and the address can be used again), its owner's access (they need a new invitation to open another store), its products, shopping bags, coupons and customer conversations; unpaid orders are cancelled and their stock returned; customers who signed up there keep their accounts. What stays, read-only, for the books: its orders, wallet ledger, deposits and withdrawals, and the audit trail — they record real payments, so they are never erased (the store remains behind them as a deleted record, and its orders show *Deleted store*).
- A store can't be deleted while it still has business: open orders (deliver, cancel or refund them), deposits waiting for review, withdrawals in progress, or money in the owner's balance (they withdraw it first — deleting would leave them unable to reach it). The dialog says exactly which. Until then, suspend it. Zendropship's own demo store can't be deleted.
