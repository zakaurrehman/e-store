...
export type OpenStoreInput = { storeName: string; slug?: string; firstName: string; lastName: string; email: string; password: string };
...
export async function openStoreForNewOwner(input: OpenStoreInput, meta: { ipAddress?: string } = {}) {
  ...
  try {
    ...
    await writeAudit({ actorId: user.id, action: "user.register", entityType: "User", entityId: user.id, summary: `Store owner registered (${store.slug})`, ipAddress: meta.ipAddress });
...
export async function openStoreForUser(userId: string, input: { storeName: string; slug?: string }) {
