...
export async function recordDepositAction(_state: ActionState, formData: FormData): Promise<ActionState> {
  const parsed = depositSchema.safeParse({ amount: formData.get("amount"), reference: formData.get("reference") ?? undefined, note: formData.get("note") ?? undefined });
  if (!parsed.success) return zodFailure(parsed.error);
  try {
    const { user, store } = await assertStoreOwner();
    const deposit = await recordDeposit({ storeId: store.id, amountCents: parsed.data.amount, reference: parsed.data.reference, note: parsed.data.note, createdById: user.id });
    revalidatePath("/dashboard");
    revalidatePath("/dashboard/balance");
