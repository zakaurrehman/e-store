...
export async function generateMetadata({ params, searchParams }: PageProps<"/s/[store]/c/[slug]">): Promise<Metadata> {
  const [{ slug }, query, store] = await Promise.all([params, searchParams, storeFromParams(params)]);
  ...
  const description = category.seoDescription ?? category.description ?? `Shop ${category.name} at ${store.name}.`;
...
async function CategoryContent({ params, searchParams }: PageProps<"/s/[store]/c/[slug]">) {
  const [{ slug }, query, { scope }] = await Promise.all([params, searchParams, storeAndScope(params)]);
  const [category, tree] = await Promise.all([getCategoryBySlug(slug), getCategoryTree(scope)]);
  if (!category) notFound();
  // Only link to sub-categories this store actually stocks.
  const stocked = new Set(tree.flatMap((department) => [department.id, ...department.children.map((child) => child.id)]));
  const children = category.children.filter((child) => stocked.has(child.id));
  ...
  return (
    <>
      ...
      <div className="mt-10">
        <ProductListing basePath={`/c/${category.slug}`} scope={{ categorySlug: category.slug }} catalog={scope} searchParams={query} hideCategoryFacet={children.length === 0} />
      </div>
    </>
  );
}

export default function CategoryPage(props: PageProps<"/s/[store]/c/[slug]">) {
  return (
