...
export async function generateMetadata({ params }: PageProps<"/s/[store]/brands">): Promise<Metadata> {
  const store = await storeFromParams(params);
  return { title: "Our labels", description: `Discover the labels available at ${store.name}.`, alternates: { canonical: "/brands" } };
}

export default function BrandsPage(props: PageProps<"/s/[store]/brands">) {
  return (
    <Suspense fallback={<div className="container-page pb-20 pt-6"><ListingSkeleton /></div>}>
      <BrandsContent {...props} />
    </Suspense>
  );
}

async function BrandsContent({ params }: PageProps<"/s/[store]/brands">) {
  const { store, scope } = await storeAndScope(params);
  const brands = (await listBrands(scope)).filter((brand) => brand.productCount > 0);
  ...
  return (
    <div className="container-page pb-20 pt-6">
      ...
      <header className="mt-6 max-w-2xl">
        ...
        <p className="mt-3 text-base leading-relaxed text-ink-600">Every label {store.name} carries, from A to Z.</p>
      </header>
