...
export async function generateMetadata({ params }: PageProps<"/s/[store]/p/[slug]">): Promise<Metadata> {
  const [{ slug }, { store, scope }] = await Promise.all([params, storeAndScope(params)]);
  const product = await getProductBySlug(slug, scope);
  if (!product) return { title: "Product not found", robots: { index: false } };
  const title = product.seoTitle ?? `${product.name}${product.brand ? ` by ${product.brand.name}` : ""}`;
  const description = product.seoDescription ?? product.shortDescription ?? `Shop ${product.name} at ${store.name}.`;
...
function productJsonLd(product: ProductDetail, currency: string, base: string) {
  ...
  return {
    ...
    image: product.images.map((image) => absolute(image.url, base)),
    ...
    offers:
      product.variants.length > 1
        ? {
            ...
            availability,
            url: absolute(`/p/${product.slug}`, base),
          }
        : {
            "@type": "Offer",
            priceCurrency: currency,
...
async function ProductContent({ params, searchParams }: PageProps<"/s/[store]/p/[slug]">) {
  const [{ slug }, query, { store, scope }] = await Promise.all([params, searchParams, storeAndScope(params)]);
  const product = await getProductBySlug(slug, scope);
  if (!product) notFound();
  ...
  return (
    <>
      <JsonLd data={productJsonLd(product, store.currency, store.url)} />
      ...
      <Suspense fallback={null}>
        <Recommendations product={product} departmentId={department?.id ?? null} scope={scope} />
      </Suspense>
...
async function Recommendations({ product, departmentId, scope }: { product: ProductDetail; departmentId: string | null; scope: StoreScope }) {
  const [together, related] = await Promise.all([
    getBoughtTogether(product.id, departmentId, 3, scope),
    getRelatedProducts(product.id, product.category?.id ?? null, product.brand?.id ?? null, 8, scope),
  ]);
...
export default function ProductPage(props: PageProps<"/s/[store]/p/[slug]">) {
  return (
    <Suspense
      fallback={
