...
async function Overview({ params, searchParams }: PageProps<"/s/[store]/account">) {
  const [user, query, store] = await Promise.all([requireUser("/account"), searchParams, storeFromParams(params)]);
  const overview = await getCustomerOverview(user.id, store.id);
  const stats = [
  ...
  return (
    <div className="space-y-12">
      {query.welcome === "1" && (
        <Alert tone="success" title={`Welcome to ${store.name}`}>
...
export default function AccountPage(props: PageProps<"/s/[store]/account">) {
  return (
