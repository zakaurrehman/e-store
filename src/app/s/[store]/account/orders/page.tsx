...
async function OrdersList({ params, searchParams }: PageProps<"/s/[store]/account/orders">) {
  const [user, query, store] = await Promise.all([requireUser("/account/orders"), searchParams, storeFromParams(params)]);
  const page = Math.max(1, Number.parseInt(typeof query.page === "string" ? query.page : "1", 10) || 1);
  const data = await listCustomerOrders(user.id, { page, storeId: store.id });
  return (
...
export default function OrdersPage(props: PageProps<"/s/[store]/account/orders">) {
  return (
