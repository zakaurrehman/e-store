...
export async function generateMetadata({ searchParams }: PageProps<"/s/[store]/search">): Promise<Metadata> {
...
async function SearchContent({ params, searchParams }: PageProps<"/s/[store]/search">) {
  const [query, { scope }] = await Promise.all([searchParams, storeAndScope(params)]);
  const filters = parseListingFilters(query);
  if (!filters.q) {
    const departments = await getCategoryTree(scope);
    return (
      <>
  ...
  const result = await listProducts({}, filters, scope);
...
export default function SearchPage(props: PageProps<"/s/[store]/search">) {
  return (
