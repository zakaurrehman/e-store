...
async function OrderDetail({ params }: PageProps<"/account/orders/[number]">) {
  const [{ number }, user] = await Promise.all([params, requireUser("/account/orders")]);
  const order = await getOrderForCustomer(user.id, number);
  if (!order) notFound();
  return (
    <div>
...
export default function OrderDetailPage(props: PageProps<"/account/orders/[number]">) {
  return (
