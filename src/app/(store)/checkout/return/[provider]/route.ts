 ...
 */
export async function GET(request: Request, { params }: RouteContext<"/checkout/return/[provider]">) {
  const { provider: providerKey } = await params;
  const url = new URL(request.url);
  const number = normaliseOrderNumber(url.searchParams.get("order") ?? "");
  const order = number ? await db.order.findUnique({ where: { number }, include: { payments: { orderBy: { createdAt: "desc" }, take: 1 } } }) : null;
