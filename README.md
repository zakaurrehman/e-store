...
- 81 unit tests and 75 integration tests pass, including the order money breakdown and commission rules, the ledger (exactly-once sale, commission and fulfilment postings under concurrent attempts, proportional refunds, pending cash on delivery, withdrawals and deposits that cannot double-spend), the funding gate, the fulfilment lifecycle (stage order, refused skips, cancelled orders, per-store isolation), invitation codes (including a race for the last use), support conversations and their privacy (including an owner's own thread with Zendropship, which stays reachable while their store is suspended), per-store logos in emails, the guest confirmation link, and phone validation.
- The Playwright suite (30 tests) passes against the development server, including the brief's full scenario: staff invite an owner, the owner opens and brands a store, prices products, a customer pays, the breakdown adds up, a below-cost order waits for funds, a confirmed deposit releases it with exactly one charge, staff take it to delivered, owner and customer follow the timeline, and customer service runs between customer, store and staff — including an owner asking Zendropship about a deposit and the two of them going back and forth in one thread.
