...
**Automation** — orders are recorded with the price paid and the wholesale cost and go straight to fulfilment; stock and catalogue prices are read live, so stores never sell what cannot ship; customers receive confirmation, shipping and delivery emails; the owner is emailed on every sale and every customer message; each sale posts its sale, Zendropship's commission (10% by default, configurable, fixed on the order) and the fulfilment cost to the owner's ledger exactly once, held until the order is delivered, and refunds post compensating entries; at acceptance the wholesale cost is set aside once — from the customer's payment when it has been collected, otherwise from the owner's available balance — and an order waits for funds only when that balance genuinely cannot cover it, going to fulfilment when a deposit is confirmed; new catalogue products appear in the demo store automatically.
