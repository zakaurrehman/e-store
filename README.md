...
**Owner dashboard** (`/dashboard`) — overview with sales, margin and a setup checklist, the store's products with wholesale / selling price / margin and per-product pricing (store rule, own markup or fixed price), show/hide and remove, orders with fulfilment status and per-order margin, customers, design and details (name, tagline, logo, homepage image and text, accent colour, announcement, support email), store-wide pricing (suggested prices or a markup), and a payments page that states plainly what is and is not available yet.
...
**Automation** — orders are recorded with the price paid and the wholesale cost and go straight to fulfilment; stock and catalogue prices are read live, so stores never sell what cannot ship; customers receive confirmation, shipping and delivery emails; the owner is emailed on every sale; new catalogue products appear in the demo store automatically.
...
**Admin** (`/admin`, role-based access enforced on the server) — everything from the single-store version (orders, customers, products, inventory, catalogue organisation, coupons, reviews, messages, content, media, settings, shipping and tax, staff and roles, audit log, import/export) plus **Stores**: every store with its owner, products, orders and sales, with suspend and reopen.
...
Checked on the development machine with the local database (2026-09-19):

- Typecheck and lint clean; production build succeeds (`next build`).
- 64 unit tests and 37 integration tests pass, including store pricing, tenancy rules, shelf isolation, per-store carts and orders, coupon scoping and store opening.
- The Playwright suite (23 tests) passes against the development server: the original storefront, checkout, account and admin flows (now running in the demo store) plus the platform journey — browse the catalogue, open a store from a product, add products, set a markup, a shopper pays in the owner's store, the owner sees the order and margin, stores stay separate, the admin suspends and reopens the store.
